...
const tabs: { svg: React.ReactElement, link: string }[] = [
  {
    svg: <svg className='stroke-current' fill='transparent' viewBox='0 0 24 24' strokeWidth={2} key={v4()}>
    ...
    svg: <svg className='fill-current' viewBox='0 0 256 259' key={v4()}>
      <g>
  ...
  return (
    <div className='fixed top-0 left-0 h-screen w-12 flex flex-col bg-gray-800'>
      <div className='flex flex-row justify-evenly items-center h-8 w-12 no-select no-drag'>
        ...
        <button className='h-full w-full flex justify-center items-center hover:bg-gray-700 title-button' onClick={(e) => { e.preventDefault(); ipcRenderer.send('nav_forth') }}>
          <svg fill='none' viewBox='0 0 24 24' stroke='currentColor' strokeWidth='2'>
