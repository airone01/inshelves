...
const tabs: React.ReactElement[] = [
  <svg className='h-6 w-6 text-white group-hover:text-black stroke-current smooth duration-200' fill='transparent' viewBox='0 0 24 24' strokeWidth={2}>
    <path strokeLinecap='round' strokeLinejoin='round' d='M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' />
  </svg>,
  <svg className='h-6 w-6 text-white group-hover:text-black stroke-current smooth duration-200' fill='transparent' viewBox='0 0 24 24' strokeWidth={2}>
    <path strokeLinecap='round' strokeLinejoin='round' d='M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4' />
  </svg>,
  <svg className='h-6 w-6 text-white group-hover:text-black fill-current smooth duration-200' viewBox='0 0 256 259'>
    <g>
  ...
  return (
    <div className='fixed top-0 left-0 h-screen w-12 flex flex-col bg-gray-800 shadow-lg'>
      ...
      <ul className='grid grid-cols-1 gap-1.5'>
        {tabs.map(e =>
          <li key={e.toString()} className='group relative h-12 w-full'>
            <div className='absolute top-0 left-0 group-hover:bg-blue-500 h-full w-full filter blur-sm smooth duration-200' />
            <button className='absolute top-0 left-0 flex justify-center items-center h-full w-full p-3 title-button bg-gray-800 group-hover:bg-blue-500 rounded-md duration-200'>
              {e}
            </button>
          </li>
        )}
